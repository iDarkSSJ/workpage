...
import { useGoogleSignIn } from "../api/useAuth"

type GoogleBtnProps = React.ButtonHTMLAttributes<HTMLButtonElement>

const CLIENT_URL = import.meta.env.VITE_CLIENT_URL

export default function GoogleBtn({ className, ...props }: GoogleBtnProps) {
  const { mutate: signInWithGoogle, isPending } = useGoogleSignIn()

  const handleGoogleSignIn = () => {
    signInWithGoogle(`${CLIENT_URL}/dashboard`)
  }
  ...
  return (
    <Button
      ...
      className={cn(
        "w-full flex items-center justify-center gap-3 border border-transparent bg-primary-bg/45 text-white hover:border-primary",
        className,
      )}
      {...props}>
      <GoogleIcon />
      {isPending ? "Conectando..." : "Google"}
    </Button>
  )
}
